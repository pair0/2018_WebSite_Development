	...
	<div class="container">
		<table class="table table-hovor">
			<thead>
				...
				<tbody>
				<%
					BbsDAO_free bbsDAO_free = new BbsDAO_free();
					ArrayList<Bbs> list = bbsDAO_free.getList(pageNumber);
					for(int i=0; i< list.size(); i++){
						if((bbs.getBbsSearch().equals(list.get(i).getBbsName())==true && bbs.getBbsSearchoption().equals("작성자")==true) || 
								(bbs.getBbsSearch().equals(list.get(i).getBbsTitle())==true && bbs.getBbsSearchoption().equals("제목")==true)){
					
				%>
				<tr>
					<td><a href="view_free.jsp?bbsID=<%=list.get(i).getBbsID()%>"> <%= list.get(i).getBbsID()+2 %> </a></td>
					<td><a href="view_free.jsp?bbsID=<%=list.get(i).getBbsID()%>"> <%= list.get(i).getBbsName() %> </a></td>
			...
			<div class="row">
			<div class="col-md-6">
			<a href="free_input.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				if(pageNumber != 1) {
			%>
				<a href="free_board_list-1.jsp?pageNumber=<%=pageNumber-1 %>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} 
				if(bbsDAO_free.nextPage(pageNumber+1)) {
			%>
				<a href="free_board_list-1.jsp?pageNumber=<%=pageNumber+1 %>" class="btn btn-success btn-arraw-right">다음</a>
			<%
				}
			%>
			</div>
			...
			<div class="col-md-6">
			<form method="post" action="free_board_list-1.jsp">
