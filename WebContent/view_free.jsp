...
<body>
	...
	<div class="container">
		<div class="row">
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
					<tbody>
						...
						<tr>
							...
							<td> <a href="fileDown_free.jsp?bbsID=<%=bbs.getBbsID()%>">  <%= bbs.getBbsFileName() %> </a></td>
						</tr>
						<tr>
				...
				<a href="free_board_list.jsp" class="btn btn-primary">목록</a>
				<%if (userID!=null){ %>
				<%
				if(userID.equals(bbs.getBbsName())==true){
				%>
				<a href="Update_free.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">수정</a>
				<a onClick="return confirm('정말로 삭제하겠습니까?')" href="deleteAction_free.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">삭제</a>
				<%} %>
				<%} %>
				
				<form action="writeAction_com_free.jsp?bbsID=<%=bbsID %>" method="post" >
     			<br>
     			<hr>
     			<p>댓글 입력</p> 
    			...
    			<table class="table table-hover">  
     			...
     			ArrayList<Bbs> list = comDAO.getList(pageNumber);
      			for(int i=0; i< list.size(); i++){
					if(list.get(i).getCombbsID()==bbsID && list.get(i).getCombbsNum() == 0 && list.get(i).getBbsAvailable()==1){
     			 %>
      			<tr>
              ...
              <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdownPortfolio">
              	 <a class="dropdown-item" href="Update_com_free.jsp?bbsID=<%=list.get(i).getBbsID()%>&bsID=<%=bbsID%>">수정</a>
                 <a class="dropdown-item" href="deleteAction_com_free.jsp?bbsID=<%=list.get(i).getBbsID()%>&bsID=<%=bbsID%>">삭제</a>
              </div>
            </a>
            </td>
      			 <%} 
      			 ...
      			 %>
      			 <td><a href="GoodAction_free.jsp?bbsID=<%=list.get(i).getBbsID()%>&bsID=<%=bbsID%>"> <img class="img-fluid" src="images/1200px-Botón_Me_gusta_svg.png" alt=""></a>&nbsp&nbsp&nbsp<%= list.get(i).getBbsGood() %></td>
      			 <td><a href="BadAction_free.jsp?bbsID=<%=list.get(i).getBbsID()%>&bsID=<%=bbsID%>"> <img class="img-fluid" src="images/1200px-Botón_Me_gusta.png" alt=""></a>&nbsp&nbsp&nbsp<%=list.get(i).getBbsBad() %></td>
      			</tr>
      			<%
      				}
     			 }
    ...
    <%
				if(pageNumber != 1) {
			%>
				<a href="view_free.jsp?pageNumber=<%=pageNumber-1 %>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} 
				if(comDAO.nextPage(pageNumber+1)) {
			%>
				<a href="view_free.jsp?pageNumber=<%=pageNumber+1 %>" class="btn btn-success btn-arraw-right">다음</a>
			<%
				}
			%>
		-->		
