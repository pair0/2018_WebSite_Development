...
<body>
	<%
		...
		{	
			BbsDAO_4 bbsDAO_4 = new BbsDAO_4();
			if(bbs.getBbsFileName()!= null)
			{
				result = bbsDAO_4.write(userID, bbs.getBbsCountry(), bbs.getBbsTitle(), bbs.getBbsMemo(), bbs.getBbsFileName(), 0);
			}
			else
			{
				result = bbsDAO_4.write(userID, bbs.getBbsCountry(), bbs.getBbsTitle(), bbs.getBbsMemo(), 0);
			}
			if( result == -1)
			{
				PrintWriter script=response.getWriter();
			...
			else 
			{
				PrintWriter script=response.getWriter();
				script.println("<script>");
				script.println("location.href = 'trap_board_list_4.jsp'");
				script.println("</script>");
			}
		}
	%>
