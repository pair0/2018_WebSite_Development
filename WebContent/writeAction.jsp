...
<body>
	<%
		...
		{	
			...
			{
				result = bbsDAO.write(userID, bbs.getBbsCountry(), bbs.getBbsTitle(), bbs.getBbsMemo(), bbs.getBbsFileName(), 0);
			}
			else
			{
				result = bbsDAO.write(userID, bbs.getBbsCountry(), bbs.getBbsTitle(), bbs.getBbsMemo(), 0);
			}
			...
			else 
			{
				PrintWriter script=response.getWriter();
				script.println("<script>");
				script.println("location.href = 'trap_board_list.jsp'");
				script.println("</script>");
			}
		}
	%>
