	...
	<div class="container">
		<table class="table table-hovor">
			...
			<%
				if(pageNumber != 1) {
			%>
				<a href="free_board_list.jsp?pageNumber=<%=pageNumber-1 %>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} 
				if(bbsDAO_free.nextPage(pageNumber+1)) {
			%>
				<a href="free_board_list.jsp?pageNumber=<%=pageNumber+1 %>" class="btn btn-success btn-arraw-right">다음</a>
			<%
				}
			%>
			</div>
